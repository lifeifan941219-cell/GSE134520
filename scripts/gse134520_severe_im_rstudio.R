# GSE134520 Severe Intestinal Metaplasia (IMS) analysis in RStudio
# Usage in RStudio:
# 1. Open this file.
# 2. Edit the CONFIG section if needed.
# 3. Run the whole file or execute sections step by step.
...
read_sample <- function(sample_input, gsm, min_cells) {
  ...
  seu <- CreateSeuratObject(counts = counts, project = gsm, min.cells = min_cells, min.features = 0)
  seu$gsm <- gsm
  seu
}
...
run_gse134520_severe_im <- function(config = CONFIG) {
  dir.create(config$cache_dir, recursive = TRUE, showWarnings = FALSE)
  dir.create(config$outdir, recursive = TRUE, showWarnings = FALSE)

  gse <- get_series_object(config$geo_id, config$cache_dir)
  sample_df <- build_sample_metadata(gse)
  write_csv(sample_df, file.path(config$outdir, "gse134520_sample_metadata_all.csv"))
  ...
  if (is.null(input_dir) || identical(input_dir, "")) {
    input_dir <- download_supp_files(config$geo_id, config$cache_dir)
  }

  search_roots <- unpack_archives(input_dir)
  manifest_path <- write_directory_manifest(search_roots, config$outdir)
  ...
  sample_objects <- lapply(severe_df$gsm, function(gsm) {
    ...
    read_sample(sample_input, gsm, min_cells = config$min_cells)
  })
  ...
  if (length(sample_objects) == 1) {
    ...
    seu <- merge(sample_objects[[1]], y = sample_objects[-1], add.cell.ids = names(sample_objects), project = config$geo_id)
  }

  seu <- subset(seu, subset = nFeature_RNA >= config$min_features)
  seu[["percent.mt"]] <- PercentageFeatureSet(seu, pattern = "^MT-")
  seu <- subset(seu, subset = percent.mt <= config$max_mt_pct)
  ...
  seu <- FindVariableFeatures(seu, selection.method = "vst", nfeatures = config$nfeatures)
  seu <- ScaleData(seu)
  seu <- RunPCA(seu, npcs = config$dims)
  seu <- FindNeighbors(seu, dims = seq_len(config$dims))
  seu <- FindClusters(seu, resolution = config$resolution)
  seu <- RunUMAP(seu, dims = seq_len(config$dims))

  markers <- FindAllMarkers(seu, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
  write_csv(markers, file.path(config$outdir, "severe_im_cluster_markers.csv"))
  write_csv(severe_df, file.path(config$outdir, "severe_im_sample_metadata.csv"))
  write_csv(seu@meta.data %>% rownames_to_column("cell_id"), file.path(config$outdir, "severe_im_cell_qc_and_clusters.csv"))
  saveRDS(seu, file.path(config$outdir, "severe_im_seurat.rds"))

  save_umap_plot(seu, "seurat_clusters", file.path(config$outdir, "umap_clusters.png"), label = TRUE)
  save_umap_plot(seu, "gsm", file.path(config$outdir, "umap_sample.png"), label = FALSE)
...
seu <- run_gse134520_severe_im(CONFIG)
